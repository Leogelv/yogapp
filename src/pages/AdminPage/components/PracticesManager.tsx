...
const PracticesManager: React.FC<PracticesManagerProps> = ({
 ...
 const startEditing = (practice: Practice, field: string) => {
   console.log(
     `Начинаем редактирование поля ${field} для практики ${practice.title}`
   );
 ...
 const saveInlineEdit = async () => {
   ...
   try {
     console.log(
       `Сохраняем изменение поля ${editingCell.field} для практики ID: ${editingCell.id}`
     );
     ...
     const cell = document.querySelector(
       `td[data-id="${editingCell.id}"][data-field="${editingCell.field}"]`
     );
     if (cell) {
 ...
 return (
   <div className="admin-section">
     ...
     {loading ? (
       ...
       <div className="admin-table-container practices-table">
             ...
             {practices.length > 0 ? (
               practices.map((practice) => (
                 <tr key={practice.id}>
                   ...
                   >
                     {practice.duration
                       ? formatTimeFromSeconds(practice.duration)
                       : '-'}
                   </td>
                   <td>
                   ...
                   <td>
                     <div className="action-buttons">
                       ...
                       <button
                         ...
                         onClick={() =>
                           console.log(
                             'Клик по кнопке "Удалить" практику:',
                             practice.title
                           )
                         }
                       >
