...
const CategoriesManager: React.FC = () => {
 ...
 const handleAddCategory = async () => {
   ...
   try {
     ...
     const newOrder = maxOrderData?.display_order
     ...
     setUpdateError(
       error.message || 'Произошла ошибка при добавлении категории'
     );
   } finally {
     setAddLoading(false);
   }
 ...
 const handleDeleteCategory = async (id: string) => {
   ...
   try {
     ...
     if (linkedPractices && linkedPractices.length > 0) {
       ...
       throw new Error(
         `Эта категория используется в практиках: ${practiceNames}`
       );
     }
     ...
     setUpdateError(
       error.message || 'Произошла ошибка при удалении категории'
     );
   } finally {
 ...
 const saveCategory = async () => {
   ...
   try {
     ...
     setUpdateError(
       error.message || 'Произошла ошибка при обновлении категории'
     );
   } finally {
     setUpdateLoading(false);
   }
 ...
 return (
   <div className="admin-section">
     ...
     {updateError && (
       <div className="admin-error admin-update-error">{updateError}</div>
     )}

     {loading ? (
       <div className="admin-loading">Загрузка категорий...</div>
     ) : (
       <div className="admin-table-container categories-table">
