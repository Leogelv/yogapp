...
import {getEventTypeLabel} from "@/pages/CalendarPage/CalendarPage.tsx";
...
export const RenderPlayer = ({content, activeType}: {content: ContentItem, activeType: string}) => {
    if (!content) {
        return <div className="player-error">Контент не найден</div>;
    }

    console.log('AutoPlayPracticePage: Рендеринг плеера, тип:', activeType, 'контент:', content?.title);
    console.log(content?.kinescope_id)
    switch (activeType) {
        ...
        case PlayerType.AUDIO:
            return (
                content.audio_file_path ? <AudioPlayer
...
const AutoPlayPracticePage: React.FC = () => {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            supabase?.from('users').select('*').eq('telegram_id', user?.id).then(r => {
                const userSupa = r.data![0]
                console.log(userSupa)
                console.log(content)
                supabase!.from('users').update({
                    power: userSupa.power + (content?.power_given || 0),
                    practice_minutes: userSupa.practice_minutes + Math.floor((content?.duration || 0)/60),
                }).eq('telegram_id', user?.id).then(res => {
                    console.log('Обновление прошло:', res);
                })
            })
        }, Math.floor((content?.duration || 0)/60) * 60 * 1000); // 10 минут
    ...
    if (eventId) {
        return (<div className={'flex flex-col gap-4'}>
            <img className="w-full h-[211px] object-cover"
                 src={content.thumbnail_url || "/assets/images/favourites-card-bg.png"} alt={content.title}/>
            <div className={'flex flex-col gap-2 !px-3'}>
                <h2 className={'text-2xl font-bold text-[#191919]'}
                    style={{fontFamily: 'RF Dewi, sans-serif', letterSpacing: '-0.07em'}}>{content.title}</h2>
                <div className={'flex items-center gap-2 flex-wrap'}>
                    <div className={'w-max text-sm !py-1 !px-2 text-white bg-[#414141] border border-[#191919]'}
                        ...
                        {getEventTypeLabel(content.event_type)}
                    </div>
                </div>
                <p className={'!mt-2 text-[#191919]'}>{content.description}</p>
            </div>
        </div>)
    }
    if (contentId) {
        return (<div className={'flex flex-col gap-4'}>
                <RenderPlayer content={content} activeType={state.activeType as string}/>
                <div className={'flex flex-col gap-2 !px-3'}>
                    <h2 className={'text-2xl font-bold text-[#191919]'}
                        style={{fontFamily: 'RF Dewi, sans-serif', letterSpacing: '-0.07em'}}>{content.title}</h2>
                    <div className={'flex items-center gap-2 flex-wrap'}>
                        <div className={'w-max text-sm !py-1 !px-2 text-white bg-[#414141] border border-[#191919]'}
                             style={{fontFamily: 'RF Dewi, sans-serif', letterSpacing: '-0.07em'}}>
                            {Number(content.power_needed) || 2} силы
                        </div>
                        <div className={'w-max text-sm !py-1 !px-2 text-white bg-[#414141] border border-[#191919]'}
                             style={{fontFamily: 'RF Dewi, sans-serif', letterSpacing: '-0.07em'}}>
                            {Math.floor(content.duration / 60)} минут
                        </div>
                        <div className={'w-max text-sm !py-1 !px-2 text-white bg-[#414141] border border-[#191919]'}
                             style={{fontFamily: 'RF Dewi, sans-serif', letterSpacing: '-0.07em'}}>
                            {content.categories?.name || 'практика'}
                        </div>
                    </div>
                    <p className={'!mt-2 text-[#191919]'}>{content.description}</p>
                </div>
                ...
                <div className="practice-nav-controls">
                    ...
                    <div className="practice-nav-right">
                        <button className="practice-control-btn">
                            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
                                <path d="M7 14l5-5 5 5" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round"
