...
## Квиз: новая архитектура (июль 2024)

- Все шаги и опции квиза теперь управляются через таблицы Supabase: `quiz_steps` (шаги) и `quiz_answers` (опции для каждого шага).
- Вся структура квиза (порядок, типы, активность шагов, варианты ответов) редактируется в реальном времени через админ-панель.
- В админке реализован новый UI для управления квизом:
  - Список шагов (quiz_steps) с возможностью добавления, редактирования, сортировки.
  - Для каждого шага — просмотр и добавление опций (quiz_answers).
  - Все изменения мгновенно отражаются на фронте (реалтайм).
- На странице /admin нижнее меню (TabBar) скрыто для максимального рабочего пространства.
- Контроль покрытия квиза ("пустые ветки") реализован через отдельный раздел: для каждой комбинации параметров можно быстро добавить недостающую практику.

### Чеклист (июль 2024)
- [x] Квиз real-time (quiz_steps, quiz_answers)
- [x] UI админки для шагов и опций квиза
- [x] Скрытие TabBar на /admin
- [x] Контроль покрытия квиза ("пустые ветки")
- [ ] Smoke-тест и финальная сверка

## UX админки (обновление)
- Все действия по наполнению квиза и практик теперь доступны в одном месте.
- Контент-менеджер видит все шаги, опции, а также "дыры" (непокрытые комбинации) и может мгновенно их закрыть.
## Квиз и рекомендации (апдейт июль 2024)
- Квиз управляется через quiz_steps/quiz_answers (real-time, редактируется через админку)
- Фильтрация и выдача практики после квиза — через таблицу contents и quiz_logic
- История рекомендаций хранится в user_recommended_contents (или user_stats, если объединим)
- Выдача практики: фильтрация по критериям квиза, исключение уже рекомендованных (история)
- Мини-попап с критериями реализуется на плеере (иконка рядом с кнопкой "Получить другую практику")
- Все шаги и варианты квиза управляются через админку, мгновенно обновляются на фронте

## Чеклист (июль 2024)
- [x] Квиз real-time (quiz_steps, quiz_answers)
- [x] Фильтрация практик по критериям (contents)
- [ ] История рекомендаций (user_recommended_contents)
- [ ] Мини-попап с критериями на плеере
- [x] Вся логика соответствует quiz_flow.md

## Последняя верификация: 14.07.2024 

## Контроль покрытия квиза и наполнение практик

### Логика
- Для каждой возможной комбинации параметров квиза (тип, цель, длительность, подход, объект) должна быть хотя бы одна практика.
- Если при прохождении квиза не найдено подходящей практики — параметры квиза логируются (и могут быть отправлены в базу для сбора статистики).

### Генерация всех комбинаций
- Используется утилита для генерации всех возможных комбинаций параметров квиза.
- Для каждой комбинации проверяется: есть ли связанная практика в quiz_logic и contents.

### Админка: раздел "Пустые ветки квиза"
- Новый раздел, где подсвечиваются все комбинации параметров, для которых нет практики.
- Для каждой пустой ветки — кнопка "Добавить практику" (открывает форму с уже проставленными параметрами).
- Позволяет быстро закрывать "дыры" в покрытии квиза.

### ТЗ для контент-менеджера
- Заполнить практиками все комбинации параметров квиза, чтобы не было "пустых" веток.
- Использовать раздел "Пустые ветки квиза" для поиска и добавления недостающих практик.
- Для каждой новой практики — сразу связывать с нужной комбинацией параметров через quiz_logic.
