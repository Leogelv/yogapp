...
6. **Интеграция с TON Connect**
   - Используется библиотека `@tonconnect/ui-react` для подключения TON кошелька
   - Реализована страница `TONConnectPage` для отображения информации о подключенном кошельке
   - Добавлен компонент `WalletBalance` для отображения баланса TON с автоматическим обновлением
   - Реализованы хелперы для работы с TON:
     - `formatTON` - преобразование наноТОНов в читабельный формат
     - `getTONWalletBalance` - получение баланса кошелька с кешированием результатов
     - `clearBalanceCache` - очистка кеша балансов
